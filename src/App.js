        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            <button onClick={() => navigateTo('home')} className="flex items-center space-x-3 group">
              <div className="w-12 h-12 bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 rounded-xl flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all group-hover:scale-105">
                <Package className="w-7 h-7 text-white" />
              </div>
              <div className="flex flex-col">
                <span className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Sreenivasa</span>
            ...
            <div className="hidden md:flex items-center space-x-8">
              {navLinks.map(link => (
                <button key={link.id} onClick={() => navigateTo(link.id)} className={`text-sm font-semibold transition-all ${currentPage === link.id ? 'text-blue-600 scale-105' : 'text-gray-700 hover:text-blue-600 hover:scale-105'}`}>
                  {link.label}
                </button>
              ))}
              <button onClick={() => navigateTo('contact')} className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-2.5 rounded-lg text-sm font-semibold hover:from-blue-700 hover:to-indigo-700 transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
                Get Quote
              </button>
            </div>
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-white border-t">
            <div className="px-4 py-4 space-y-3">
              {navLinks.map(link => (
                <button key={link.id} onClick={() => navigateTo(link.id)} className={`block w-full text-left px-4 py-2 rounded-lg text-sm font-medium transition-colors ${currentPage === link.id ? 'bg-blue-50 text-blue-600' : 'text-gray-700 hover:bg-gray-50'}`}>
                  {link.label}
                </button>
              ))}
            </div>
...
const HomePage = ({ navigateTo }) => (
  <div className="pt-20">
    <section className="relative bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button onClick={() => navigateTo('products')} className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-8 py-4 rounded-xl font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-1 flex items-center justify-center space-x-2">
                <span>Browse Products</span>
                <ArrowRight className="w-5 h-5" />
              </button>
              <button onClick={() => navigateTo('contact')} className="bg-white text-blue-600 px-8 py-4 rounded-xl font-semibold border-2 border-blue-600 hover:bg-blue-50 transition-all shadow-md">
                Request Quote
              </button>
            </div>
          </div>
          <div className="relative">
            <div className="aspect-square bg-gradient-to-br from-blue-500 via-indigo-600 to-purple-700 rounded-3xl shadow-2xl flex items-center justify-center">
              <Package className="w-48 h-48 text-white/30" />
            </div>
            <div className="absolute -bottom-6 -left-6 bg-white p-6 rounded-2xl shadow-2xl border-4 border-blue-100">
              <div className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">99.8%</div>
    ...
    <section className="py-20 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <button onClick={() => window.location.href = 'tel:+919866485661'} className="inline-flex items-center bg-white text-blue-600 px-8 py-4 rounded-xl font-semibold hover:bg-gray-100 shadow-lg">
...
const ProductsPage = () => {
  ...
  const products = [
    { name: 'Digital Blood Pressure Monitor', category: 'Diagnostic', price: '₹8,999', image: 'bg-gradient-to-br from-blue-400 to-blue-600', popular: true },
    { name: 'Surgical Scissors Set', category: 'Surgical', price: '₹4,799', image: 'bg-gradient-to-br from-green-400 to-green-600' },
    { name: 'Disposable Gloves (Box of 100)', category: 'PPE', price: '₹699', image: 'bg-gradient-to-br from-purple-400 to-purple-600', popular: true },
    { name: 'Stethoscope Professional', category: 'Diagnostic', price: '₹5,699', image: 'bg-gradient-to-br from-blue-400 to-blue-600' },
    { name: 'Surgical Masks (Box of 50)', category: 'PPE', price: '₹549', image: 'bg-gradient-to-br from-purple-400 to-purple-600' },
    { name: 'Medical Gauze Pads', category: 'Supplies', price: '₹349', image: 'bg-gradient-to-br from-orange-400 to-orange-600' },
    { name: 'Digital Thermometer', category: 'Diagnostic', price: '₹1,299', image: 'bg-gradient-to-br from-blue-400 to-blue-600' },
    { name: 'Bandage Rolls', category: 'Supplies', price: '₹229', image: 'bg-gradient-to-br from-orange-400 to-orange-600' }
  ];
    ...
    <div className="pt-20">
      ...
      <section className="py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap gap-3 mb-8 justify-center">
            {categories.map(cat => (
              <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-6 py-3 rounded-xl font-semibold transition-all ${selectedCategory === cat ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg' : 'bg-gray-100 text-gray-700'}`}>
                {cat}
              </button>
            ))}
          </div>
              ...
              <div key={idx} className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all overflow-hidden transform hover:-translate-y-2">
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold">Inquire</button>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="py-16 bg-gradient-to-br from-blue-50 to-indigo-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <button onClick={() => window.location.href = 'tel:+919866485661'} className="inline-flex items-center bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-8 py-4 rounded-xl font-semibold shadow-lg">
            <Phone className="w-5 h-5 mr-2" />
  ...
  <div className="pt-20">
    ...
    <section className="py-16 bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <button onClick={() => navigateTo('contact')} className="inline-block bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-8 py-4 rounded-xl font-semibold shadow-lg">
          Contact Our Team
        </button>
      </div>
    </section>
            ...
            <div className="grid grid-cols-2 gap-6 mt-8">
              {[{ n: '20+', l: 'Years' }, { n: '5000+', l: 'Clients' }, { n: '10000+', l: 'Products' }, { n: '99.8%', l: 'Satisfaction' }].map((s, i) => (
                <div key={i} className="text-center p-4 bg-blue-50 rounded-xl">
                  <div className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">{s.n}</div>
          ...
          <div className="grid md:grid-cols-2 gap-12">
            <div>
              ...
              <div className="space-y-6">
                <input type="text" name="name" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="Full Name" className="w-full px-4 py-3 border rounded-lg" required />
                <input type="email" name="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} placeholder="Email" className="w-full px-4 py-3 border rounded-lg" required />
                <input type="tel" name="phone" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} placeholder="Phone" className="w-full px-4 py-3 border rounded-lg" />
                <input type="text" name="company" value={formData.company} onChange={(e) => setFormData({...formData, company: e.target.value})} placeholder="Company" className="w-full px-4 py-3 border rounded-lg" />
                <textarea name="message" value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} placeholder="Message" rows="4" className="w-full px-4 py-3 border rounded-lg" required></textarea>
                <button type="button" onClick={handleSubmit} className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-8 py-4 rounded-lg font-semibold shadow-lg">
                  Send Message
                </button>
              </div>
            </div>
        ...
        <div>
          ...
          <div className="flex space-x-3">
            {[Facebook, Twitter, Linkedin, Instagram].map((Icon, i) => (
              <button key={i} className="w-9 h-9 bg-gray-800 rounded-lg flex items-center justify-center hover:bg-blue-600 transition-colors">
